import streamlit as st
import numpy as np
import cv2
import os
import tensorflow as tf
from streamlit_webrtc import webrtc_streamer, VideoTransformerBase

# Clear any existing TensorFlow sessions at startup
tf.keras.backend.clear_session()

# Set page configuration
st.set_page_config(page_title="Emotion Detection App", layout="wide")

# Title and description
st.title("Real-time Emotion Detection")
st.write("Choose a model and start detecting emotions!")

# Model selection radio button
model_choice = st.radio(
    "Select the model for emotion detection:",
    ("CNN Model", "ViT-CNN Model")
)

# Dictionary to label emotion categories
emotion_dict = {
    0: "Angry", 1: "Disgust", 2: "Fear", 3: "Happy",
    4: "Neutral", 5: "Sad", 6: "Surprise"
}

def get_model():
    """Load and compile the selected model."""
    # Clear session before loading a new model
    tf.keras.backend.clear_session()
    try:
        if model_choice == "CNN Model":
            model_path = 'Trained_Model.h5'
        else:
            model_path = 'emotion_vit_cnn_model.h5'
        
        with tf.keras.utils.custom_object_scope({}):
            model = tf.keras.models.load_model(model_path, compile=False)
            model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
        return model
    except Exception as e:
        st.error(f"Error loading model: {str(e)}")
        return None

@st.cache_resource
def load_face_cascade():
    """Load the Haar cascade for face detection."""
    try:
        cascade_path = os.path.join(cv2.data.haarcascades, 'haarcascade_frontalface_default.xml')
        return cv2.CascadeClassifier(cascade_path)
    except Exception as e:
        st.error(f"Error loading face cascade: {str(e)}")
        return None

# Initialize face cascade in session state if not already loaded
if 'face_cascade' not in st.session_state:
    st.session_state['face_cascade'] = load_face_cascade()
    
if st.session_state['face_cascade'] is None:
    st.error("Failed to load face detection model. Please check your installation.")
    st.stop()

# Check if model is loaded or if model choice has changed
if 'previous_model_choice' not in st.session_state:
    st.session_state['previous_model_choice'] = model_choice
    st.session_state['model'] = get_model()
elif st.session_state['previous_model_choice'] != model_choice:
    # Reload the model if the selection changes
    st.session_state['model'] = get_model()
    st.session_state['previous_model_choice'] = model_choice

# Define the video transformer class for processing frames
class EmotionVideoTransformer(VideoTransformerBase):
    def __init__(self, model, face_cascade, emotion_dict):
        self.model = model
        self.face_cascade = face_cascade
        self.emotion_dict = emotion_dict

    def transform(self, frame):
        # Convert the input frame to a numpy array (BGR)
        img = frame.to_ndarray(format="bgr24")
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        
        # Detect faces in the grayscale frame
        faces = self.face_cascade.detectMultiScale(
            gray,
            scaleFactor=1.1,
            minNeighbors=5,
            minSize=(30, 30)
        )
        
        for (x, y, w, h) in faces:
            # Draw a rectangle around each detected face
            cv2.rectangle(img, (x, y), (x+w, y+h), (0, 255, 0), 2)
            roi_gray = gray[y:y+h, x:x+w]
            roi_gray = cv2.resize(roi_gray, (48, 48), interpolation=cv2.INTER_AREA)
            
            if np.sum([roi_gray]) != 0:
                # Normalize and reshape the ROI for the model
                roi = roi_gray.astype('float') / 255.0
                roi = np.expand_dims(roi, axis=-1)
                roi = np.expand_dims(roi, axis=0)
                
                try:
                    prediction = self.model.predict(roi, verbose=0)[0]
                    emotion_label = self.emotion_dict[np.argmax(prediction)]
                    confidence = float(np.max(prediction) * 100)
                    label_text = f"{emotion_label} ({confidence:.1f}%)"
                    cv2.putText(img, label_text, (x, y-10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                except Exception as e:
                    cv2.putText(img, "Error", (x, y-10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        return img

# Factory function to create an instance of the video transformer
def transformer_factory():
    return EmotionVideoTransformer(
        model=st.session_state['model'],
        face_cascade=st.session_state['face_cascade'],
        emotion_dict=emotion_dict
    )

# Start the client-side webcam using webrtc_streamer
st.header("Real-time Emotion Detection using Webcam")
webrtc_streamer(key="emotion", video_transformer_factory=transformer_factory)

# Sidebar and information columns for guidance
st.sidebar.markdown(f"### Current Model: {model_choice}")
st.sidebar.info("Webcam access is handled on the client side. Please allow camera access when prompted.")

info_col1, info_col2 = st.columns(2)
with info_col1:
    st.markdown("""
    ### Instructions:
    1. Select your preferred model.
    2. Allow webcam access when prompted.
    3. Watch real-time emotion detection.
    4. Refresh the page to switch models.
    """)
with info_col2:
    st.markdown("""
    ### Detected Emotions:
    - 😠 Angry
    - 🤢 Disgust
    - 😨 Fear
    - 😊 Happy
    - 😐 Neutral
    - 😢 Sad
    - 😲 Surprise
    """)
